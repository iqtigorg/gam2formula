#' Derive closed formula for a P-spline smooth
#'
#' @param smooth A \code{"ps"} (P-spline) smooth.
...
#'
#' @return A nested tibble with name of the term, type of the smooth (\code{"ps"}) and a closed formula representation of the smooth.
smooth2formula_ps <- function(smooth, mod, ngrid = 1e3){
  ...
  # get spline degree (note: for some reason we need to add one for bs="ps")
  m <- smooth$p.order[1] + 1
  # get knots
  knots <- smooth$knots
  # get number of knots
  nk <- length(knots)
  ...
  default_knots <- smooth.construct(s(x, bs = "ps", m = m - 1, k = smooth$bs.dim),
